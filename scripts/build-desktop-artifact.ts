...
interface StagePackageJson {
  ...
  readonly t3codeBuildTimestamp: string;
...
const DMG_BACKGROUND_CHANNEL = "latest" as const;
...
export function resolveDesktopBuildIconAssets(version: string): DesktopBuildIconAssets {
  if (isNightlyDesktopVersion(version)) {
    return {
...
export const createBuildConfig = Effect.fn("createBuildConfig")(function* (
  ...
  if (platform === "mac" && target === "dmg") {
    buildConfig.dmg = {
      ...
      // The fork ships no nightly channel, so the themed installer always uses
      // the latest-channel background.
      background: `dmg/dmg-background-${DMG_BACKGROUND_CHANNEL}.png`,
      window: {
        width: 540,
...
const buildDesktopArtifact = Effect.fn("buildDesktopArtifact")(function* (
  ...
  if (options.platform === "mac" && options.target === "dmg") {
    yield* stageDesktopDmgBackground(stageResourcesDir, DMG_BACKGROUND_CHANNEL, options.verbose);
  }
