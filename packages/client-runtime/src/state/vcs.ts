...
export function createVcsEnvironmentAtoms<R, E>(
  ...
  // Flat for the same reason as listRefsFamily above: a nested per-environment
  // family can be collected between lookups, dropping the cached answer.
  const listWorktreesFamily = Atom.family((key: string) => {
    const [environmentId, input] = JSON.parse(key) as [EnvironmentId, VcsListWorktreesInput];
    return runtime
      .atom((get) => {
      ...
      .pipe(
        Atom.setIdleTTL(VCS_REFS_IDLE_TTL_MS),
        Atom.withLabel(`environment-data:vcs:list-worktrees:${key}`),
      );
  });
  const listWorktrees = (target: {
    readonly environmentId: EnvironmentId;
    readonly input: VcsListWorktreesInput;
  }) => listWorktreesFamily(JSON.stringify([target.environmentId, target.input]));
