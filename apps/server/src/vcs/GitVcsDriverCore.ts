...
const INDEX_LOCK_CONTENTION_PATTERN = /Unable to create '(?:[^']*[\\/])?index\.lock': File exists/i;
const INDEX_LOCK_PATH_PATTERN = /Unable to create '((?:[^']*[\\/])?index\.lock)': File exists/i;
...
export const makeGitVcsDriverCore = Effect.fn("makeGitVcsDriverCore")(function* () {
  ...
  const executeRaw: GitVcsDriver.GitVcsDriver["Service"]["execute"] = Effect.fnUntraced(
    function* (input) {
      ...
      const scopedRun = runGitCommand().pipe(Effect.scoped);
      const runAttempt =
        timeoutMs === null
          ? scopedRun
          : scopedRun.pipe(
      ...
      const finalizeResult = (result: GitVcsDriver.ExecuteGitResult) =>
        !input.allowNonZeroExit && result.exitCode !== 0
          ? Effect.fail(
              new GitCommandError({
                ...gitCommandContext(commandInput),
      ...
      const recovered =
        subcommand !== undefined && commandHoldsIndexLock(subcommand, input.env)
          ? getRepoWriteLock(commandInput.cwd).pipe(
              Effect.flatMap((writeLock) => writeLock.withPermit(runWithLockRecovery(0))),
            )
          : runWithLockRecovery(0);
      return yield* recovered.pipe(Effect.flatMap(finalizeResult));
    },
  );
  ...
  const deleteBranch: GitVcsDriver.GitVcsDriver["Service"]["deleteBranch"] = Effect.fn(
    ...
    if (isRemoteRef || input.deleteRemote) {
      ...
      if (remoteName && remoteBranch) {
        ...
        const result = yield* executeGit("GitVcsDriver.deleteBranch.remote", input.cwd, pushArgs, {
          timeoutMs: 30_000,
          allowNonZeroExit: true,
        });
        if (result.exitCode === 0) {
          ...
          return yield* new GitCommandError({
            ...gitCommandContext({
              operation: "GitVcsDriver.deleteBranch.remote",
              cwd: input.cwd,
              args: pushArgs,
            }),
            detail: result.stderr.trim() || "git push --delete failed",
          });
        }
      }
    }
  ...
  return GitVcsDriver.GitVcsDriver.of({
    ...
    fetch: (input) => withListRefsInvalidation(input.cwd, fetch(input)),
