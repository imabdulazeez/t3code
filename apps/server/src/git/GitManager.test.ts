    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(ghCalls).toContain(
          "pr list --head statemachine --state all --limit 100 --json number,title,url,baseRefName,headRefName,state,isDraft,mergedAt,closedAt,updatedAt,isCrossRepository,headRepository,headRepositoryOwner --repo jasonLaster/codething-mvp",
        );
      }),
    20_000,
  );
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(ghCalls).toContain(
          "pr list --head main --state all --limit 100 --json number,title,url,baseRefName,headRefName,state,isDraft,mergedAt,closedAt,updatedAt,isCrossRepository,headRepository,headRepositoryOwner --repo contributor/codething-mvp",
        );
      }),
    20_000,
  );
