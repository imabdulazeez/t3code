...
it.layer(GitManagerTestLayer)("GitManager", (it) => {
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(ghCalls).toContain(
          "pr list --head jasonLaster:statemachine --state all --limit 20 --json number,title,url,baseRefName,headRefName,state,isDraft,mergedAt,closedAt,updatedAt,isCrossRepository,headRepository,headRepositoryOwner --repo jasonLaster/codething-mvp",
        );
      }),
    20_000,
  );
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(ghCalls).toContain(
          "pr list --head contributor:main --state all --limit 20 --json number,title,url,baseRefName,headRefName,state,isDraft,mergedAt,closedAt,updatedAt,isCrossRepository,headRepository,headRepositoryOwner --repo contributor/codething-mvp",
        );
      }),
    20_000,
  );
  ...
  it.effect("carries recent commit subjects as policy examples, not instructions", () =>
    Effect.gen(function* () {
      const repoDir = yield* makeTempDir("t3code-git-manager-");
      ...
      yield* runGit(repoDir, ["commit", "-m", "fix(web): trim sidebar labels"]);
      NodeFS.writeFileSync(NodePath.join(repoDir, "README.md"), "hello\nagain\n");
      yield* runGit(repoDir, ["add", "README.md"]);
      let generatedPolicy: TextGeneration.CommitMessageGenerationInput["policy"] = undefined;
      ...
      expect(generatedPolicy).toEqual({
        ...
        conventionExamples: expect.stringContaining("fix(web): trim sidebar labels"),
