...
export default function DiffPanel({
  ...
  useEffect(() => {
    ...
    if (fileSearchMatchPaths.length > 0) {
      revealDiffFile(fileSearchMatchPaths[0]!);
    }
  }, [fileSearchMatchPaths, revealDiffFile]);
    ...
    <>
      ...
      <div className="flex shrink-0 items-center gap-2 [-webkit-app-region:no-drag]">
        <div className="flex items-center gap-1">
          <SearchIcon className="pointer-events-none size-3 shrink-0 text-muted-foreground/60" />
          <Input
            size="compact"
            className="w-40"
            ...
            onKeyDown={(e) => {
              if (e.key === "Enter" && fileSearchMatchPaths.length > 0) {
                ...
                revealDiffFile(fileSearchMatchPaths[fileSearchMatchIndexRef.current]!);
              }
            }}
          />
        </div>
