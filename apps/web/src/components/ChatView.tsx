...
export default function ChatView(props: ChatViewProps) {
  ...
  const rightPanelState = useRightPanelStore((store) =>
    selectThreadRightPanelState(store.byThreadKey, activeThreadRef),
  );
  ...
  const createBrowserSurface = useCallback(() => {
    ...
    if (!api) return;
    void api.preview
      .open({ threadId: activeThreadRef.threadId })
      .then((snapshot) => {
  ...
  const handleToolbarTerminalScopeChange = useCallback(
    (scope: ProjectScriptScope) => selectTerminalScope(scope),
    [selectTerminalScope],
  );
  ...
  const activateRightPanelSurface = useCallback(
    (surface: RightPanelSurface) => {
      ...
      if (surface.kind === "terminal" && activeThreadOwnerRef) {
        setTerminalFocusRequestId((value) => value + 1);
      }
  ...
  const closeRightPanelSurface = useCallback(
    ...
    [activeThreadOwnerRef, activeThreadRef, diffOpen, environmentId, navigate, threadId],
  );
        ...
        <div className="flex min-h-0 min-w-0 flex-1 flex-col overflow-x-hidden">
          ...
          {activeThreadRef ? (
            <PersistentConsolidatedTerminalDrawer
              ...
              visible={terminalUiState.terminalOpen || projectTerminalState.terminalOpen}
