...
describe("uiStateStore pure functions", () => {
  ...
  it("stores changed-file overrides that differ from the configured default", () => {
    const threadId = ThreadId.make("thread-1");
    ...
    const expanded = setThreadChangedFilesExpanded(makeUiState(), threadId, "turn-2", true, false);
  ...
  it("removes changed-file overrides when expansion matches the configured default", () => {
    const threadId = ThreadId.make("thread-1");
    const collapsed = makeUiState({
      threadChangedFilesExpandedById: {
        [threadId]: {
          "turn-1": false,
        },
    ...
    expect(
      setThreadChangedFilesExpanded(expanded, threadId, "turn-2", false, false)
        .threadChangedFilesExpandedById,
    ).toEqual({});
  });
